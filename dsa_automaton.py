...
def init_grid():
    ...
    # Padding to prevent out-of-bounds; padding has value 1 - solid obstacle
    np_obstacles_padded = np.pad(np_obstacles, 
                                 pad_width=PAD, 
                                 mode='constant', 
                                 constant_values=1)
    ...
    # Replace logical 1 with actual value DR_HERE
    np_drones[np_drones == 1] = DR_HERE
    np_drones[obst_mask] = DR_NONE
    # Padding to prevent out-of-bounds; padding has value DR_NONE
    np_drones_padded = np.pad(np_drones, 
                              pad_width=PAD, 
                              mode='constant', 
                              constant_values=DR_NONE)
    
    # Layer 2: visits
    # No need for a bitmap, just all zeros
    np_visits = np.zeros_like(np_obstacles).astype(CELL_TYPE)
    # Mark all cells with obstacles as unreachable
    np_visits[obst_mask] = V_UNREACHABLE
    # Padding to prevent out-of-bounds; padding has value V_UNREACHABLE
